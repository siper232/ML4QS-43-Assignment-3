...
DATASET_FNAME = 'chapter2_result_goat.csv'
RESULT_FNAME =  'chapter3_result_outliers_goat.csv'
...
DATA_PATH = Path('./intermediate_datafiles/')
DATASET_FNAME = 'chapter5_result_goat.csv'
...
dftest = adfuller(dataset['ax'], autolag='AIC')

plt.Figure(); autocorrelation_plot(dataset['ax'])
DataViz.save(plt)
plt.show()
...
DataViz.plot_numerical_prediction_versus_real(train_X.index, train_y, regr_train_y['ax'], test_X.index, test_y, regr_test_y['ax'], 'ax')
...
train_X, test_X, train_y, test_y = prepare.split_single_dataset_regression(copy.deepcopy(dataset), ['ax', 'ay'], 0.9, filter=False, temporal=True)

output_sets = learner.dynamical_systems_model_nsga_2(train_X, train_y, test_X, test_y, ['self.ax', 'self.ay', 'self.az'],
                                                     ['self.a * self.ax + self.b * self.ay', 'self.c * self.ay + self.d * self.az', 'self.e * self.ax + self.f * self.az'],
                                                     ['self.ax', 'self.ay'],
...
regr_train_y, regr_test_y = learner.dynamical_systems_model_ga(train_X, train_y, test_X, test_y, ['self.ax', 'self.ay', 'self.az'],
                                                     ['self.a * self.ax + self.b * self.ay', 'self.c * self.ay + self.d * self.az', 'self.e * self.ax + self.f * self.az'],
                                                     ['self.ax', 'self.ay'],
                                                     ['self.a', 'self.b', 'self.c', 'self.d', 'self.e', 'self.f'],
                                                     pop_size=5, max_generations=10, per_time_step=True)

DataViz.plot_numerical_prediction_versus_real(train_X.index, train_y['ax'], regr_train_y['ax'], test_X.index, test_y['ax'], regr_test_y['ax'], 'ax')

regr_train_y, regr_test_y = learner.dynamical_systems_model_sa(train_X, train_y, test_X, test_y, ['self.ax', 'self.ay', 'self.az'],
                                                     ['self.a * self.ax + self.b * self.ay', 'self.c * self.ay + self.d * self.az', 'self.e * self.ax + self.f * self.az'],
                                                     ['self.ax', 'self.ay'],
                                                     ['self.a', 'self.b', 'self.c', 'self.d', 'self.e', 'self.f'],
