...
class CreateDataset:
    ...
    def combine_datasets(self, fileNames):
        dataset = []
        for file in fileNames:
            df = pd.read_csv(self.base_dir / file)
            
            # Add tables 
            df['timestamp'] = pd.to_datetime(df['YYYY-MO-DD HH-MI-SS_SSS'])
            df['label'] = file.split('.')[0]

            # Clean labels
            dataset['label'] = dataset['label'].apply(self.clean_name)

            # Remove unwanted columns
            df.drop(['YYYY-MO-DD HH-MI-SS_SSS', 'Time since start in ms'], axis=1)

            dataset.append(df)
        self.combined_data_table = pd.concat(dataset, axis=0, ignore_index=True) #, axis=0, ignore_index=True