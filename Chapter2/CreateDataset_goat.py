...
class CreateDataset:
    ...
    def add_numerical_dataset(self, file, timestamp_col,datasetsize, value_cols, aggregation='avg', prefix=''):
        print(f'Reading data from {file}')
        dataset = pd.read_csv(self.base_dir / file, skipinitialspace=True,nrows=datasetsize)

        # Convert timestamps to dates
        dataset[timestamp_col] = dataset[timestamp_col] * 1000000 + 1654560005000000000
        ...
        for i in range(0, len(self.data_table.index)):
            ...
            self.data_table.loc[self.data_table.index[i], "label"] = max(list(relevant_rows['label']))
            for col in value_cols:
                ...
                if len(relevant_rows) > 0:
                    if aggregation == 'avg':
                        ...
                        #np.where(relevant_rows['label'] == 'walking', 1, 0).size
                    else:
